...
std::vector<CameraPose> relpose_mono_3pt_wrapper(const std::vector<Eigen::Vector2d> &x1,
                                             const std::vector<Eigen::Vector2d> &x2, const std::vector<Eigen::Vector2d> &x3) {
    std::vector<CameraPose> output;
    essential_3pt_mono_depth(x1, x2, x3, &output);
    return output;
}
...
std::pair<CameraPose, py::dict> estimate_relative_pose_w_mono_depth_wrapper(const std::vector<Eigen::Vector2d> points2D_1,
                                                               const std::vector<Eigen::Vector2d> points2D_2,
                                                               const std::vector<Eigen::Vector2d> sigmas,
                                                               const py::dict &camera1_dict,
                                                               const py::dict &camera2_dict,
                                                               const py::dict &ransac_opt_dict,
                                                               const py::dict &bundle_opt_dict) {
    ...
    std::vector<char> inlier_mask;
    RansacStats stats =
        estimate_relative_pose_w_mono_depth(points2D_1, points2D_2, sigmas, camera1, camera2, ransac_opt, bundle_opt, &pose, &inlier_mask);
    py::dict output_dict;
    write_to_dict(stats, output_dict);
    output_dict["inliers"] = convert_inlier_vector(inlier_mask);
    return std::make_pair(pose, output_dict);
...
estimate_shared_focal_relative_pose_wrapper(const std::vector<Eigen::Vector2d> points2D_1,
    ...
    RansacStats stats = estimate_shared_focal_relative_pose(points2D_1, points2D_2, ransac_opt, bundle_opt,
...
std::pair<ImagePair, py::dict>
estimate_shared_focal_monodepth_relative_pose_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                            const std::vector<Eigen::Vector2d> &points2D_2,
    ...
    std::vector<Image> output;
    RansacStats stats = estimate_shared_focal_monodepth_relative_pose(points2D_1, points2D_2, sigma, ransac_opt,
...
std::pair<ImagePair, py::dict>
estimate_varying_focal_monodepth_relative_pose_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                                       const std::vector<Eigen::Vector2d> &points2D_2,
                                                       const std::vector<Eigen::Vector2d> &sigma,
                                                       const py::dict &ransac_opt_dict,
                                                       const py::dict &bundle_opt_dict) {
    ...
    std::vector<Image> output;
    RansacStats stats = estimate_varying_focal_monodepth_relative_pose(points2D_1, points2D_2, sigma, ransac_opt,
                                                                       bundle_opt, &image_pair, &inlier_mask);
...
PYBIND11_MODULE(poselib, m) {
    py::class_<poselib::CameraPose>(m, "CameraPose")
        ...
        .def_readwrite("shift", &poselib::CameraPose::shift)
    ...
    m.def("relpose_mono_3pt", &poselib::relpose_mono_3pt_wrapper, py::arg("x1"), py::arg("x2"), py::arg("depth"));
    ...
    m.def("estimate_relative_pose_w_mono_depth", &poselib::estimate_relative_pose_w_mono_depth_wrapper, py::arg("points2D_1"),
    ...
    m.def("estimate_shared_focal_monodepth_relative_pose", &poselib::estimate_shared_focal_monodepth_relative_pose_wrapper,
          py::arg("points2D_1"), py::arg("points2D_2"), py::arg("sigma"),
          py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(),
          "Relative pose estimation with unknown equal focal lengths using depth information with non-linear refinement.");
    m.def("estimate_varying_focal_monodepth_relative_pose", &poselib::estimate_varying_focal_monodepth_relative_pose_wrapper,
          py::arg("points2D_1"), py::arg("points2D_2"), py::arg("sigma"),
          py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(),
          "Relative pose estimation with unknown equal focal lengths with non-linear refinement.");
